...
static float f_Cell_Content_left;
    ...
    /// 单元格 左边字体   [默认:[UIFont systemFontOfSize:15.f]]
static UIFont *font_Cell_Left;
    /// 单元格内容 字体   [默认:[UIFont systemFontOfSize:15.f]]
...
- (void)createSectionView:(NSInteger)section startRow:(NSInteger)startRow
{
    ...
    if (self.hasBorder) {
        ...
        [backView setBorderWidth:0.5 coror:COLOR_LINE];
    ...
    for (NSInteger i = 0; i < rowCount; i++) {
        ...
        if (self.showReplace && self.editable) {
            tTextField.placeholder = [NSString stringWithFormat:@"请输入%@", [self.titles keyAtIndex:currRow]];
        }
...
//        }
        ...
        if (i != rowCount - 1) {
            ...
            tImgView.frame = CGRectMake(0, tSectionH + tCellH - tImage.size.height, tSectionW , tImage.size.height);
            [tImgView setImage:tImage];
            if (!tImage) {
                tImgView.height = 0.5;
                tImgView.backgroundColor = COLOR_LINE;
            }
            
            if (self.hasBorder) {
                tImgView.frame = CGRectMake(f_Section_bg_shadow_w, tSectionH + tCellH - tImage.size.height, tSectionW - f_Section_bg_shadow_w * 2, tImage.size.height);
            }
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    if ((keyboard == UIKeyboardTypeNumberPad) || (keyboard == UIKeyboardTypePhonePad)) {
...
- (void)keyboardWillChange:(NSNotification *)notification
{
    ...
    float boardH = keyboardRect.size.height+45;
...
+ (void)defaultSetting
{
    img_Section_Bg = nil;//[[UIImage imageNamed:@"round_bg_1@2x.png"] resizableImage:6 left:6 bottom:6 right:6];
    img_Cell_Line = [UIImage imageNamed:@"line_h2_a"];

    f_Section_bg_shadow_w = 2.f / 2;
    f_Section_Top = 15 / 2.f;
    f_Section_Left = 15 / 2.f;
    f_Cell_h = 44.f;
    f_Cell_Content_left = 10.f;
    f_Cell_Left_w = 85.f;
    f_Cell_Max_Left_w = 0.f;

    font_Cell_Left = [UIFont systemFontOfSize:12.f];
    font_Cell_Content = [UIFont systemFontOfSize:13.f];

    color_Cell_Left = [ZXPublic getColor:@"888888"];
    color_Cell_Content = COLOR_BLACK;
}
