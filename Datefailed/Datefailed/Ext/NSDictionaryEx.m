...
- (NSString *)getNullTitle:(NSInteger)index
{
    ...
    if (!value || [value isEqualToString:@""]) {
        return [@"请填写" stringByAppendingString:[self.allKeys objectAtIndex:index]];
    } else if ([value rangeOfString:@"请选择"].location != NSNotFound) {
        return [@"请选择" stringByAppendingString :[self.allKeys objectAtIndex:index]];
    }
...
- (NSString *)getUnNumberTitleAtIndex:(NSInteger)index
{
    ...
    if (![value isNumber]) {
        return [@"请填写正确的" stringByAppendingString :[self.allKeys objectAtIndex:index]];
    }

    return nil;
}

-(NSString *) getUnFloatTitleAtIndex:(NSInteger)index
{
    if (self.count <= index) {
        return nil;
    }
    ...
    //判断是否是纯数字
    if( ![value isPureInt] && ![value isPureFloat])
    {
        return [@"请填写正确的" stringByAppendingString :[self.allKeys objectAtIndex:index]];
    }
    return nil;
...
- (id)objectByClass:(Class)tClass;
{
    ...
    for (id key in [self allKeys]) {
        ...
        else if ([value respondsToSelector:NSSelectorFromString([@"_" appendStr:key])]) {
            [value setValue:[self objectForKey:key] forKey:[@"_" appendStr:key]];
        }
    }
...
@implementation NSMutableDictionary(MutableExt)

- (void)autoAddObject:(id)anObject forKey:(id <NSCopying>)aKey;
{
    if (!anObject) {
        anObject = @"";
    }
